let rawData = [];
let selectedMakes = []; // Marcas selecionadas
...
async function loadData() {
    ...
    // Preencher botões de marca
    const makeButtonsDiv = document.getElementById('makeButtons');
    const makes = [...new Set(rawData.map(d => d.Make))].sort();
    selectedMakes = [...makes]; // Todas selecionadas inicialmente

    makeButtonsDiv.innerHTML = '';
    makes.forEach(make => {
        const btn = document.createElement('button');
        btn.textContent = make;
        btn.className = 'make-btn active';
        btn.style = `
            padding: 6px 14px;
            border-radius: 20px;
        ...
        btn.onclick = function() {
            if (selectedMakes.includes(make)) {
                selectedMakes = selectedMakes.filter(m => m !== make);
        ...
        makeButtonsDiv.appendChild(btn);
    });
...
function filterData() {
    ...
    return rawData.filter(row => {
        ...
        const matchesMake = selectedMakes.length === 0 ? false : selectedMakes.includes(row.Make);
...
controlsDiv.innerHTML = `
    <div style="margin: 20px 0; padding: 15px; background-color: #ecf0f1; border-radius: 8px; display: flex; flex-wrap: wrap; gap: 20px;">
        ...
        <div style="display: flex; flex-direction: column; min-width: 200px;">
            ...
            <div id="makeButtons" style="display: flex; flex-wrap: wrap; gap: 8px;"></div>
        </div>
        <div style="display: flex; flex-direction: column; min-width: 200px;">
...
// Remova o evento do select de marca (não existe mais)
